...
CREATE FUNCTION "public"."all_meets_admin"("userid" integer) RETURNS TABLE("id" integer, "happening" timestamp with time zone, "topic" character varying, "location" character varying, "tags" "jsonb", "images" "jsonb", "rsvp" "jsonb", "questions" bigint)
LANGUAGE "plpgsql"
AS $$
BEGIN
...
END; $$;
CREATE FUNCTION "public"."all_meets_user"("userid" integer) RETURNS TABLE("id" integer, "happening" timestamp with time zone, "topic" character varying, "location" character varying, "tags" "jsonb", "images" "jsonb", "rsvp" character varying, "questions" bigint)
LANGUAGE "plpgsql"
AS $$
BEGIN
...
json4 jsonb := json1;
BEGIN
FOR rec in SELECT * FROM jsonb_array_elements_text(json1)
LOOP
IF json2 ? rec.value THEN
...
AS $$
BEGIN
OLD.tags := json_merge(OLD.tags, NEW.tags);
NEW.dImages := json_merge(OLD.images, NEW.images);
RETURN NEW;
END; $$;
...
"created" timestamp with time zone DEFAULT CURRENT_TIMESTAMP(0) NOT NULL,
"dimages" "jsonb" DEFAULT '[]'::"jsonb",
CONSTRAINT "meet_happening_check" CHECK (("happening" > "now"()))
);
